  ...
  return (
    <div className="relative">
      ...
      <button
        ref={buttonRef}
        onClick={() => setOpen((p) => {
          if (p) setExpandedId(null); // reset expanded when closing
          return !p;
        })}
                ...
                <div className="py-1">
                  {items.map((item) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-3 transition-colors duration-150 border-b border-white/[0.04] last:border-0 hover:bg-white/[0.04] cursor-pointer ${
                        item.is_read && expandedId !== item.id ? 'opacity-50' : ''
                      }`}
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-2">
                            <p className={`text-xs font-medium leading-snug ${item.is_read && expandedId !== item.id ? 'text-text-muted' : 'text-text'}`}>
                              {item.title}
                            </p>
                          ...
                          <p className={`text-xs text-text-muted mt-0.5 leading-relaxed transition-all duration-200 ${
