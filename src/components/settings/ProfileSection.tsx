...
export default function ProfileSection() {
  ...
  // Read language reactively from auth store
  const userLang = useAuthStore((s) => s.user?.language || 'en');
  const [lang, setLang] = useState(userLang);
  const [savingLang, setSavingLang] = useState(false);

  // Sync local state when store updates (e.g. after reload/refreshUser)
  useEffect(() => {
    setLang(userLang);
  }, [userLang]);

  const handleLangChange = async (code: string) => {
    setLang(code); // optimistic update
    setSavingLang(true);
    try {
      await apiClient('/api/webapp/user/profile', {
        method: 'POST',
      ...
      setLang(userLang); // revert to store value on error
    } finally {
      setSavingLang(false);
    }
  };
